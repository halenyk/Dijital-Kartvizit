...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm']) && $_POST['confirm'] === 'yes') {
    try {
        $stmt = $db->prepare("DELETE FROM kartlar1 WHERE kart_id = ?");
        $stmt->execute([$kart_id]);
        header('Location: kartlar.php?message=success');
        exit();
...
try {
    $stmt = $db->prepare("SELECT kart_baslik FROM kartlar1 WHERE kart_id = ?");
    $stmt->execute([$kart_id]);
...
<li class="menu-item">
 ...
 <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Kartlar /</span> Kart Sil</h4>
        <div class="row">
            <div class="col-md">
                <div class="card mb-4">
                    <h5 class="card-header">Kart Sil</h5>
                    <div class="card-body">
                        <p><?php echo htmlspecialchars($kart['kart_baslik']); ?> başlıklı kartı silmek istediğinizden emin misiniz?</p>
                        <form action="kart_sil.php?id=<?php echo $kart_id; ?>" method="POST">
                            <input type="hidden" name="confirm" value="yes">
                            <button type="submit" class="btn btn-danger">Evet, Sil</button>
                            <a href="kartlar.php" class="btn btn-secondary">İptal</a>
                        </form>
                    </div>
                </div>
            </div>
