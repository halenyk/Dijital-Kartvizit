...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $iban = $_POST['iban'];
    $iban_kisi = $_POST['iban_kisi'];
    $banka_logo = $_POST['mevcut_logo'];

    if (isset($_FILES['banka_logo']) && $_FILES['banka_logo']['error'] == 0) {
        $logo_filename = basename($_FILES['banka_logo']['name']);
        $logo_path = 'img/' . $logo_filename;
        $logo_path2 = '../../img/' . $logo_filename;

        if (move_uploaded_file($_FILES['banka_logo']['tmp_name'], __DIR__ . '/' . $logo_path)) {
            copy(__DIR__ . '/' . $logo_path, __DIR__ . '/' . $logo_path2);
            $banka_logo = $logo_path;
        } else {
            $error = 'Logo yükleme başarısız oldu.';
        }
    }

    if (empty($error)) {
        try {
            $stmt = $db->prepare("UPDATE banka_hesaplari SET banka_ad = ?, iban = ?, iban_kisi = ?, banka_logo = ? WHERE banka_id = ?");
            $stmt->execute([$banka_ad, $iban, $iban_kisi, $banka_logo, $banka_id]);
            $success = true;
...
try {
    $stmt = $db->prepare("SELECT * FROM banka_hesaplari WHERE banka_id = ?");
...
<li class="menu-item">
   ...
   <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Banka Hesapları /</span> Banka Hesabı Düzenle</h4>
        <div class="row">
            <div class="col-md">
                <div class="card mb-4">
                    <h5 class="card-header">Banka Hesabı Düzenle</h5>
                    <?php if ($success): ?>
                    ...
                    <div class="card-body pt-4">
                        <form action="banka_duzenle.php?id=<?php echo $banka_id; ?>" method="POST" enctype="multipart/form-data">
                            <div class="form-group mb-3">
                                <label for="banka_ad" class="form-label">Banka Adı</label>
                                <input type="text" class="form-control" id="banka_ad" name="banka_ad" value="<?php echo htmlspecialchars($banka['banka_ad']); ?>" required>
                            </div>
                            <div class="form-group mb-3">
                                <label for="iban" class="form-label">IBAN</label>
                                <input type="text" class="form-control" id="iban" name="iban" value="<?php echo htmlspecialchars($banka['iban']); ?>" required>
                            </div>
                            <div class="form-group mb-3">
                                <label for="iban_kisi" class="form-label">IBAN Sahibi</label>
                                <input type="text" class="form-control" id="iban_kisi" name="iban_kisi" value="<?php echo htmlspecialchars($banka['iban_kisi']); ?>" required>
                            </div>
                            <div class="form-group mb-3">
                                <label for="banka_logo" class="form-label">Banka Logosu</label>
                                <input type="file" class="form-control" id="banka_logo" name="banka_logo" accept="image/*">
                                <?php if ($banka['banka_logo']): ?>
                                    <img src="<?php echo htmlspecialchars($banka['banka_logo']); ?>" alt="Banka Logosu" width="100" style="margin-top: 10px;">
                                    <input type="hidden" name="mevcut_logo" value="<?php echo htmlspecialchars($banka['banka_logo']); ?>">
                                <?php endif; ?>
                            </div>
