...
try {
    $stmt = $db->prepare("SELECT COUNT(*) FROM kartlar1");
    $stmt->execute();
    $total_kartlar = $stmt->fetchColumn();
    $total_pages = ceil($total_kartlar / $limit);

    $stmt = $db->prepare("SELECT * FROM kartlar1 LIMIT :limit OFFSET :offset");
...
<body>
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Kartlar /</span> Tüm Kartlar</h4>
        ...
        <div class="d-flex justify-content-start mb-3">
            <a href="kart_ekle.php" class="btn btn-primary">
                <i class="bx bx-plus me-1"></i> Kart Ekle
            </a>
        </div>
        <div class="card">
            <div class="table-responsive text-nowrap">
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0" id="kartTableBody">
                        <?php if (count($kartlar) > 0): ?>
                            <?php foreach ($kartlar as $kart): ?>
                                <tr>
                                    <td><?php echo htmlspecialchars(truncate($kart['kart_baslik'], 20)); ?></td>
                                    <td><?php echo htmlspecialchars(truncate($kart['kart_icerik'], 20)); ?></td>
                                    <td>
                                        <a href="<?php echo htmlspecialchars($kart['kart_link']); ?>" target="_blank" class="truncate" title="<?php echo htmlspecialchars($kart['kart_link']); ?>">
                                            <?php echo htmlspecialchars(truncate($kart['kart_link'], 20)); ?>
                                        </a>
                                    </td>
                                    <td><img src="<?php echo htmlspecialchars($kart['kart_resim']); ?>" alt="Kart Resmi" width="50"></td>
                                    <td>
                                        <a href="kart_duzenle.php?id=<?php echo $kart['kart_id']; ?>" class="btn btn-primary btn-sm">
                                            <i class="bx bx-edit-alt me-1"></i> Düzenle
                                        </a>
                                        <a href="kart_sil.php?id=<?php echo $kart['kart_id']; ?>" class="btn btn-danger btn-sm">
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="5" class="text-center">Hiç kart bulunamadı.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-center mt-4">
                    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                        <li class="page-item <?php if ($i == $page) echo 'active'; ?>"><a class="page-link" href="kartlar.php?page=<?php echo $i; ?>"><?php echo $i; ?></a></li>
                    <?php endfor; ?>
                </ul>
            </nav>
        <?php endif; ?>
