...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['profil_resmi']) && $_FILES['profil_resmi']['error'] == 0) {
        $resim = 'img/' . basename($_FILES['profil_resmi']['name']);
        $resim2 = '../../img/' . basename($_FILES['profil_resmi']['name']);
        if (move_uploaded_file($_FILES['profil_resmi']['tmp_name'], __DIR__ . '/' . $resim)) {
            copy(__DIR__ . '/' . $resim, __DIR__ . '/' . $resim2);
        } else {
            $error = 'Profil resmi yükleme başarısız oldu.';
        }
    } else {
        $resim = $_POST['mevcut_resim'];
    }

    if (empty($error)) {
        $stmt = $db->prepare("UPDATE kullanicilar SET isim = ?, meslek = ?, eposta = ?, telefon = ?, konum = ?, profil_resmi = ?, aciklama = ? WHERE id = ?");
        $stmt->execute([$isim, $meslek, $eposta, $telefon, $konum, $resim, $aciklama, $id]);
        $success = true;
    }
}
...
<li class="menu-item">
    ...
    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Bilgiler /</span>Kişisel Bilgiler</h4>
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <ul class="nav nav-pills flex-column flex-md-row mb-3">
                ...
                <li class="nav-item">
                    <a class="nav-link" href="girisbilgilerim.php"><i class="bx bxs-id-card"></i> Giriş Bilgileri</a>
                </li>
            </ul>
                ...
                <div class="card-body">
                    <form id="formAccountSettings" method="POST" enctype="multipart/form-data">
                        <div class="d-flex align-items-start align-items-sm-center gap-4">
                            <img
                                src="<?php echo htmlspecialchars($kullanici['profil_resmi']); ?>"
                                alt="user-avatar"
                            ...
                            <div class="button-wrapper">
                                <label for="upload" class="btn btn-primary me-2 mb-4" tabindex="0">
                                    <span class="d-none d-sm-block">Profil R. Yükle</span>
                                ...
                                <input type="hidden" name="mevcut_resim" value="<?php echo htmlspecialchars($kullanici['profil_resmi']); ?>">
                                <p class="text-muted mb-0">İzin verilen JPG, GIF veya PNG. Maksimum boyut 800KB</p>
                            </div>
                        </div>
                        <hr class="my-0" />
                        <div class="row">
                            <div class="mb-3 col-md-6">
                                ...
                                <input class="form-control" type="text" id="isim" name="isim" value="<?php echo htmlspecialchars($kullanici['isim']); ?>" required autofocus />
                            </div>
                            <div class="mb-3 col-md-6">
                                <label for="meslek" class="form-label">Meslek</label>
                                <input class="form-control" type="text" id="meslek" name="meslek" value="<?php echo htmlspecialchars($kullanici['meslek']); ?>" required />
                            </div>
                            <div class="mb-3 col-md-6">
                                <label for="eposta" class="form-label">E-Posta</label>
                                <input class="form-control" type="email" id="eposta" name="eposta" value="<?php echo htmlspecialchars($kullanici['eposta']); ?>" required />
                            </div>
                            <div class="mb-3 col-md-6">
                                <label for="telefon" class="form-label">Telefon</label>
                                <input class="form-control" type="text" id="telefon" name="telefon" value="<?php echo htmlspecialchars($kullanici['telefon']); ?>" required />
                            </div>
                            <div class="mb-3 col-md-6">
                                <label for="konum" class="form-label">Konum</label>
                                <input class="form-control" type="text" id="konum" name="konum" value="<?php echo htmlspecialchars($kullanici['konum']); ?>" required />
                            </div>
                            <div class="mb-3 col-md-12">
                                <label for="aciklama" class="form-label">Açıklama</label>
                                <textarea class="form-control" id="aciklama" name="aciklama" rows="4"><?php echo htmlspecialchars($kullanici['aciklama']); ?></textarea>
                            </div>
                        </div>
                        <div class="mt-2">
